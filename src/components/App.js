...
class App extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      vehicles: 2,
      zip: '',
      dwelling_type: 'Stand-alone house',
    };
  }
  ...
  calculateIfValid = () => {
    if (this.costAvailable() && this.valid()) {
      this.calculate();
    }
  };
  // TODO: convert to context API
  setAttribute = event => {
    this.setState({ [event.target.name]: event.target.value }, this.calculateIfValid);
  };
  setLoading = loading => {
  ...
  calculate = e => {
    if (this.state.zip) {
      ...
      $('.pre_calculate')
        .removeClass('pre_calculate')
        .addClass('post_calculate');
      ...
      Promise.all([
        ...
        data_other
          ? axios.post(impact_study_url, JSON.stringify(data_other), {
              responseType: 'json',
              headers: { 'Content-Type': 'application/json' },
            })
          : Promise.resolve(),
        ...
        .catch(function(error) {
          ...
          $('.search_failed').fadeIn('slow', function() {
            if (!costAvailable) {
              $('.calculate_footer').animate({ opacity: 1 });
            }
          });

          $('.calculating').fadeOut('slow', function() {
            $('.post_calculate')
              .addClass('pre_calculate')
              .removeClass('post_calculate');
            setLoading(false);
          });
  ...
  render() {
    return (
      <div id="impact_calculator">
        {process.env['SHOW_MENU'] && <Menu />}
        <Introduction />
        <LivingSituation setAttribute={this.setAttribute} />
        <FamilyInfo
          handleChange={this.setAttribute}
        ...
        <HomeInfo
          ...
          dwelling_type={this.state.dwelling_type}
          heating_type={this.state.heating_type}
        ...
        <ResultsIndicator
          net_impact={this.state.net_impact}
          show_footer_impact={this.state.show_footer_impact}
